...
class Bizyhood_Seo
{
  ...
  public function bizyhood_create_all_urls($verified = false) {
      ...
      if ($max_entries > $apimax) {
        ...
        $queryapi = Bizyhood_ApiCalls::businesses_information($query_params);
    ...
    function bizyhood_addtoindex_sitemap() {
      ...
      $getfirstpage = Bizyhood_ApiCalls::businesses_information(array('paged' => 1, 'verified' => TRUE, 'ps' => Bizyhood_Core::API_MAX_LIMIT));
    ...
    function bizy_add_aioseo_pages( $pages ) {
      ...
      $queryapi = Bizyhood_ApiCalls::businesses_information(array('paged' => 1, 'verified' => TRUE, 'ps' => Bizyhood_Core::API_MAX_LIMIT));
      ...
      while($i <= $numofpages) {
        $queryapi = Bizyhood_ApiCalls::businesses_information(array('paged' => $i, 'verified' => TRUE, 'ps' => Bizyhood_Core::API_MAX_LIMIT));
        foreach($queryapi['businesses'] as $business) {
          $urlarr = array_slice(explode('/', $business->bizyhood_url), -3);
          $pages[] = Array( "loc" => $urlbase.$urlarr[0].'/'.$urlarr[1].'/', "lastmod" => $date, "changefreq" => "weekly", "priority" => "0.6" );
        }
